        	...
        	<div class="col-md-3" role="complementary">	
				...
				<div id="main" class="blog col-md-8" role="main">
					...
					foreach($user_details as $user){ 
					 ...
					 $user_data  = get_user_by( 'login', $user->uname);
    ...
    <div class="event clearfix bg">
        ...
        <div class="entry-content cf event-content">
           <h2 class="title"><a href="<?php bloginfo('template_directory');?>/personal-profile.php?uname=<?php echo $user->uname; ?>" title = "<?php echo $user->uname; ?>"><?php if($user->name) { echo $user->name; } else { echo $user->uname; } ?></a></h2>
				  ...
				  ?>
				  <a href="<?php echo get_template_directory_uri(); ?>/delete/delete_user.php?uname=<?php echo $user->uname; ?>"><button class="">Delete</button></a> | 	
				  <a href="<?php echo get_template_directory_uri(); ?>/user-new1.php?uname=<?php echo $user->uname; ?>"><button class="">Edit</button></a>
					<br/><label style="float : right; padding-right : 5px;"> <?php if(wpsnipp_get_last_login($user_data->id)) { echo 'Last Login : '. date("Y-m-d h:i:s a", wpsnipp_get_last_login($user_data->id)); } ?> </label>
					<?php } ?>
					
    </div> 
	
	<?php // echo '<p>Previous: Login date: ' . date("Y-m-d h:i:s a", wpsnipp_get_last_login($current_user->ID,true)) . '</p>'; ?>
</article>
		<?php }}?>		
				</div>
			</div>
