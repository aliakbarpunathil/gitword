...
<form method="post" class="validate" action="<?php echo get_template_directory_uri(); ?>/wp-event.php" enctype="multipart/form-data">
...
<table class="form-table col-md-9">
	...
	<tr class="form-field">
		<th scope="row"><label for="first_name"><?php _e('Image') ?><span class="description"><?php _e('(required) - Max Size : 300 KB'); ?></span> </label></th>
		<td>
		<input type="hidden" value="<?php echo $content_post->guid; ?>" name="image">
		<input type="file" name="eventImage" id="fileToUpload" class=" customMargin " <?php  if(!isset($_GET['id'])){ ?> required <?php } ?>>
	...
	<tr class="form-field">
		<th scope="row"><label for="upload_form"><?php _e('Upload Registration Form') ?><span class="description"><?php _e('(required) - Max Size : 300 KB'); ?></span> </label></th>
		<td>
		<input type="file" name="upload_form" id="register_form" class=" customMargin " <?php  if(!isset($_GET['id'])){ ?> required <?php } ?>>
